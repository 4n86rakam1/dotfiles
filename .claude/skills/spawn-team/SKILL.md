...
| Name | Role | Worktree | Does what |

Under the table put one line on why this split and not another, and one line on anything the user must decide. Nothing else — no prompt drafts, no restated background. Launch only after approval.
...
cd <repo> && claude --bg -n <name> -w <name> \
  --permission-mode acceptEdits "$(cat <prompt-file>)"
```

`--bg` prints a short id (8 hex chars) that `claude attach|logs|stop|rm` take.

Give read-only sessions (a verifier, say) a `-w` too. That keeps an `acceptEdits` session out of the user's working copy.
...
| Pitfall                                                                                                                   | What to do                                                                                                                                                                                                                                                                                         |
| ------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A background session has nobody to answer a permission prompt and stops there the moment one appears                      | Always pass `--permission-mode acceptEdits`. Bash calls outside the settings allowlist can still stall it. This surfaces: `claude agents --json` shows that session as `status: "waiting"`, so check that before reading `claude logs <id>` for the cause                                          |
...
- **`bypassPermissions`.** `acceptEdits` is enough. Launching a session more permissive than the one you are in routes around a decision the user made about your session, so anything above `acceptEdits` needs their explicit say-so
